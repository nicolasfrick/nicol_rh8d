...
obj_points[0,:] = np.array([-length/2, length/2, 0])
obj_points[1,:] = np.array([length/2, length/2, 0])
obj_points[2,:] = np.array([length/2, -length/2, 0])
obj_points[3,:] = np.array([-length/2, -length/2, 0])
...
def invPersp(tvec: np.ndarray, rvec: np.ndarray=None, axis_angle: bool=True, rot_mat: np.ndarray=None) -> Tuple[cv2.typing.MatLike, cv2.typing.MatLike]:
...
def compute_orientation_error(R1, R2):
	...
	R_error = R1.T @ R2  # Relative rotation
	angle_error = np.arccos((np.trace(R_error) - 1) / 2)
	return angle_error

cnt = 0
def residuals(camera_pose: np.ndarray, tag_poses: dict, marker_poses: dict, err: float) -> np.ndarray:
	"""Compute the residual (error) between world and detected poses.
	Rotations are extrinsic XYZ Euler angles."""
	global cnt
	cnt += 1
	...
	for id, tag_pose in tag_poses.items():
		...
		if det is not None:
			...
			# estimated tag pose wrt camera frame
			det_mat = pose2Matrix(det['ftrans'], det['frot'])
			T_world_estimated_tag = camera_mat @ det_mat

			# Position error (Euclidean distance)
			position_error = np.linalg.norm(T_world_estimated_tag[:3, 3] - T_world_tag[:3, 3])

			# Orientation error (angle between rotations)
			R_world_tag = T_world_tag[:3, :3]
			R_world_estimated_tag = T_world_estimated_tag[:3, :3]
			R_error = R_world_estimated_tag.T @ R_world_tag  # Relative rotation
			orientation_error = np.arccos((np.trace(R_error) - 1) / 2)

			# Append errors
			# error.append(position_error)  # position error
			# error.append(orientation_error)  # orientation error
	default_err = np.array(2*len(marker_poses.keys())*[np.random.uniform(-err,err)])
	error = np.hstack(error if len(error) else default_err)
	print(error)
	return error
...
# world to marker per measure and detection
m_tvec = det[idx]['tvec']
m_mat = det[idx]['rot_mat']
T_cam_marker = pose2Matrix(m_tvec, rot_mat=m_mat)
T_world_marker_det = T_world_cam @ T_cam_marker # wTc.cTm = wTm
world_marker_det_tvec = T_world_marker_det[:3, 3]
world_marker_det_rvec = R.from_matrix(T_world_marker_det[:3, :3] ).as_euler('xyz')
if verbose:
	print("world marker detected", world_marker_det_tvec, world_marker_det_rvec)

# world to marker per measurement
r_tvec = marker_table_poses['root']['xyz']
r_euler = marker_table_poses['root']['rpy']
T_world_root = pose2Matrix(r_tvec, r_euler)
w_tvec = marker_table_poses[idx]['xyz']
w_euler = marker_table_poses[idx]['rpy']
T_root_marker = pose2Matrix(w_tvec, w_euler)
T_world_marker_meas = T_world_root @ T_root_marker # wTm
world_marker_meas_tvec = T_world_marker_meas[:3, 3]
world_marker_meas_rvec = R.from_matrix(T_world_marker_meas[:3, :3] ).as_euler('xyz')
if verbose:
	print("world to marker measured", world_marker_meas_tvec, world_marker_meas_rvec)

# world to cam per detected and measured tf
inv_det_tvec, inv_det_rot_mat = invPersp(m_tvec, rot_mat=m_mat)
T_marker_cam = pose2Matrix(inv_det_tvec, rot_mat=inv_det_rot_mat)
T_world_cam_det = T_world_marker_meas @ T_marker_cam # wTm.mTc = wTc
world_cam_tvec = T_world_cam_det[:3, 3]
world_cam_rvec = R.from_matrix(T_world_cam_det[:3, :3] ).as_euler('xyz')
if verbose:
	print("world to cam measured and detected", world_cam_tvec, world_cam_rvec)
	print("measured cam tf", c_tvec, R.from_matrix(c_mat).as_euler('xyz'))

# initial guess
estimate = np.zeros(6)
estimate[:3] = world_cam_tvec[:]
estimate[3:] = world_cam_rvec[:]
err = 500
lower_bounds = [-3, -3, -3, -np.pi, -np.pi, -np.pi]
upper_bounds = [3, 3, 3, np.pi, np.pi, np.pi]
# estimate cam pose from world
res = least_squares(residuals, 
					estimate, 
					args=(marker_table_poses, det, err),
					method='trf', 
					bounds=(lower_bounds, upper_bounds),
					max_nfev=700,  # Max 1000 iterations
					ftol=1e-8,      # Small tolerance for the cost function
					xtol=1e-8,      # Small tolerance for the solution parameters
					gtol=1e-8       # Small tolerance for the gradient
					)
if res.success:
	estimate = res.x
	status = res.status 
	print(f"Result: {status}, camera world pose trans: {estimate[:3]}, rot (extr. xyz euler): {estimate[3:]}")
else:
	print("fail")

# invert world to camera tf for reprojection
estimate_tvec_inv, estimate_rvec_inv = invPersp(estimate[:3], estimate[3:], axis_angle=False)
T_world_cam_estimate_inv = pose2Matrix(estimate_tvec_inv, estimate_rvec_inv)
# get corner world poses 
world_corners = tagWorldCorners(T_world_marker_meas, obj_points)
projected_corners, _ = cv2.projectPoints(world_corners, T_world_cam_estimate_inv[:3, :3], T_world_cam_estimate_inv[:3, 3], cmx, dist)
projected_corners = np.int32(projected_corners).reshape(-1, 2)
cv2.polylines(img, [projected_corners], isClosed=True, color=(0,255,0), thickness=2)
corners = det[idx]['corners']
err = reprojectionError(corners, projected_corners)
if verbose:
	print(world_corners)
	print(projected_corners)
	print(corners)
print("id", idx, "repr err", err, "cnt", cnt)
